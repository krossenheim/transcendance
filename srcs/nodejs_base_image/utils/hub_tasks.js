...
async function subscribe_online_status_handler(subscriptionRequestBody) {
    if (subscriptionRequestBody.subscribe === undefined) {
        console.error("No subscription request in subscriptionRequestBody from " +
            subscriptionRequestBody.containerFrom);
        return new MessageFromService(httpStatus.BAD_REQUEST, null, "subscribe_online_status", {
    ...
    if (Array.isArray(subscriptionRequestBody.subscribe)) {
        ...
        if (current_val === 1)
            return new MessageFromService(httpStatus.OK, null, "subscribe_online_status", {
        ...
        if (subscriptionRequestBody.replace) {
            ...
            for (const user_id of subscriptionRequestBody.subscribe) {
                if (servicesSubscribedToUsers[subscriptionRequestBody.containerFrom].includes(user_id))
                    continue;
        ...
        console.log("servicesSubscribedToUsers for " +
            subscriptionRequestBody.containerFrom +
            " now: " +
            servicesSubscribedToUsers[subscriptionRequestBody.containerFrom]);
        return new MessageFromService(httpStatusToReturn, null, "subscribe_online_status", {
            message,
        });
    }
        ...
        console.error("Invalid subscription request in subscriptionRequestBody from " +
            subscriptionRequestBody.containerFrom);
        return new MessageFromService(httpStatus.BAD_REQUEST, null, "subscribe_online_status", {
...
async function unsubscribe_online_status_handler(subscriptionRequestBody) {
    if (subscriptionRequestBody.subscribe === undefined) {
        console.error("No subscription request in subscriptionRequestBody from " +
            subscriptionRequestBody.containerFrom);
        return new MessageFromService(httpStatus.BAD_REQUEST, null, "unsubscribe_online_status", {
            error: "No subscription request",
        });
    }
    if (Array.isArray(subscriptionRequestBody.subscribe)) {
        ...
        for (const user_id of subscriptionRequestBody.subscribe) {
            const index = servicesSubscribedToUsers[subscriptionRequestBody.containerFrom].indexOf(user_id);
            if (index < 0)
                return new MessageFromService(httpStatus.BAD_REQUEST, null, "unsubscribe_online_status", {
                    error: "User id " +
                        user_id +
        ...
        return new MessageFromService(httpStatus.OK, null, "unsubscribe_online_status", {
...
async function getHubTaskOutput(messageFromService) {
    ...
    for (const [task_name, task] of tasksForHub) {
        if (messageFromService.endpoint !== task.url)
            continue;
        console.log("Running task '" + task_name + "', " + messageFromService.toString());
        const result = await task.handler(messageFromService);
        if (!result) {
            console.error("Result for url/endpoint handler returned falsy value:'" + result + "'");
        }
        return result;
    }
}
module.exports = { tasksForHub, servicesSubscribedToUsers, getHubTaskOutput };
//# sourceMappingURL=hub_tasks.js.map