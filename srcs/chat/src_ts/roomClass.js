...
class Room {
    ...
    addUser(client_request) {
        ...
        if (!user_id) {
            throw new Error(`No user_id, request was: ${JSON.stringify(client_request)}`);
        }
        if (room_name != this.room_name) {
            console.error(`Room name doesn't match the requested name, request was: ${JSON.stringify(client_request)}`);
            throw new Error(`Room name doesn't match the requested name, request was: ${JSON.stringify(client_request)}`);
        }
        if (!added_user_id || added_user_id === user_id) {
            console.log(`missing or bad added_user_id, request was: ${JSON.stringify(client_request)}`);
            return {
        ...
        if (!this.users.includes(user_id)) {
            console.log(`Userid ${user_id} is not in room ${this.room_name}, request was: ${JSON.stringify(client_request)}`);
            return {
                status: httpStatus.BAD_REQUEST,
                recipients: [user_id],
                func_name: process.env.FUNC_POPUP_TEXT,
    ...
    sendMessage(client_request) {
        ...
        if (!this.users.includes(user_id)) {
            console.log(`Userid ${user_id} is not in room ${this.room_name},users in it are: [${this.users.join(', ')}], request was: ${JSON.stringify(client_request)}`);
            return {
                status: httpStatus.BAD_REQUEST,
                recipients: [user_id],
                func_name: process.env.FUNC_POPUP_TEXT,
            ...
            console.log(`Userid ${user_id} joined room ${this.room_name}, request was: ${JSON.stringify(client_request)}`);
            return {
                status: httpStatus.OK,
                recipients: this.users || [],
                func_name: process.env.FUNC_ADD_MESSAGE_TO_ROOM,
